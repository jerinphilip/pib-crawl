...
from .utils import clean_translation, detok, lazy_load, split_and_wrap_in_p
...
@docstore.route("/")
def index():
    entries = db.session.query(M.Entry).order_by(M.Entry.final_link_count.desc()).all()
...
def listing():
    ...
    x = db.session.query(M.Entry).filter(M.Entry.lang == lang).all()
...
def parallel_align():
    ...
    def process(src_out, tgt_out):
        ...
        def wrap_pairwise(src, tgt):
            def create_individual(pair):
                ...
                fmt = '<div class="row" style="border:1px solid black; margin-bottom:1em;"><div class="col-6">{}</div><div class="col-6">{}</div></div>'.format(
                    src_line, tgt_line
                )
                return fmt
    ...
    query = M.Translation.query.filter(
        and_(M.Translation.parent_id == src, M.Translation.model == model)
    ).first()
...
def stored_translations(entry_id):
    ...
    translations = M.Translation.query.filter(
        M.Translation.parent_id == entry_id, M.Translation.lang == "hi"
    ).all()
    ...
    return render_template(
        "stored_translations.html", entry=entry, translations=translations
    )
