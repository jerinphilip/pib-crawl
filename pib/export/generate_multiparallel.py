...
def collect(xx, yy, input_dir, pwriter):
    ...
    dyy = dirname(yy)
    fname = "train"
    with open("./{}/{}/{}.{}".format(input_dir, dxx, fname, xx)) as src1, open(
        "./{}/{}/{}.en".format(input_dir, dxx, fname)
    ) as tgt1, open("./{}/{}/{}.{}".format(input_dir, dyy, fname, yy)) as src2, open(
        "./{}/{}/{}.en".format(input_dir, dyy, fname)
    ) as tgt2:
...
def get_stats(langs, input_dir, fpath, fname, stats_output):
    ...
    for direction in list(perm):
        ...
        train_file_path = os.path.join(
            active_dir, canonical_lang_pair_dirname(xx, yy), train_file_name
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input-dir",
        help="input directory containing xx-en, xx-yy also gets written to this directory",
        required=True,
    )
    parser.add_argument(
        "--fpath", help="path to write multilingual data", required=True
    )
    parser.add_argument("--fname", help="name to be written", required=True)
    parser.add_argument(
        "--stats-output", help="Outputs stats file as a CSV", required=True
    )
